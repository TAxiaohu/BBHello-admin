...
(function () {
  ...
  angular
    ...
    .controller('consultingEditController', consultingEditController);

  /* @ngInject */
  function consultingEditController($state,$stateParams,toastr,config,FileUploader,dateFilter,UploaderService,EditorUploaderService,ConsultingService) {
    var vm = this;
    var queue= [];
    vm.cancel = cancel;
    vm.submit = submit;
...
//时间控件初始化
    ...
    //发送信息
    function submit(consulting) {
      var releaseTime = consulting.publicTime;
      var publicTime = dateFilter(releaseTime, 'yyyy-MM-dd');
      var displayOrder = consulting.displayOrder;
      if(displayOrder == ''){
        displayOrder = 0
      }
      ...
      var _params = angular.extend({}, consulting,params);
      ConsultingService.update(_params).then(function(res){
        if(res.code === '400'){
          ...
          $state.go('app.consulting.list');
          toastr.success('资讯修改成功');
        }
      });
    }
